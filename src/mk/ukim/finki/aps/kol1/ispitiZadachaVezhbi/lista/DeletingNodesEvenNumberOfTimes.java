package mk.ukim.finki.aps.kol1.ispitiZadachaVezhbi.lista;
...
public class DeletingNodesEvenNumberOfTimes {

    public static void transforList(SLL<Integer> list) {


        //1->10->2->3->5->2->10->3->3
        //1->3->5->3->3
        SLLNode<Integer> prv = list.getFirst();
        int broj = 1,brojZaBrishenje=0;
        boolean [] niza = new boolean[list.length()*20];

        while (prv != null) {
            SLLNode<Integer> vtor = prv.succ;
            while (vtor != null) {
                if(prv.element.equals(vtor.element)) {
                    brojZaBrishenje = prv.element;
                    broj++;
                }
                vtor = vtor.succ;
            }

            if(broj%2 == 0 && !niza[brojZaBrishenje]) {
                vtor = prv.succ;
                while (vtor != null) {
                    if(prv.element.equals(vtor.element)) {
                        list.delete(vtor);
                ...
                SLLNode<Integer> tmp = prv;
                niza[brojZaBrishenje] = true;
                prv = prv.succ;
                list.delete(tmp);
            } else {
                niza[brojZaBrishenje] = true;
