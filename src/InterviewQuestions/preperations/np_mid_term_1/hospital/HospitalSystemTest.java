package InterviewQuestions.preperations.np_mid_term_1.hospital;
...
class HospitalSystem<T extends Patient & Comparable<T>> {
    ...
    public void printResults() {
        Comparator<Patient> patientComparator = Comparator.comparing(Patient::getCode);

        //todo: define specific service implementations with lambda expressions
        PatientService<T> findAllWithDoctor = (patientList, doctorName) -> patientList.stream()
                .filter(patient -> patient.getDoctor().equals(doctorName))
                .collect(Collectors.toCollection(ArrayList::new));

        PatientService<T> findAllWithDoctor2 = new PatientService<T>() {
            @Override
            public ArrayList<T> patientsInformation(ArrayList<T> patients, String doctorName) {
        ...
        PatientService<T> countAllWithGenderAndAgeLessThanSixty = (patientList, gender) -> patientList.stream()
                .filter(patient -> patient.getAge() <= 60 && patient.getGender().equals(gender))
                .collect(Collectors.toCollection(ArrayList::new));

        PatientService<T> findAllWithNameSorted = (patientList, name) -> patientList.stream()
