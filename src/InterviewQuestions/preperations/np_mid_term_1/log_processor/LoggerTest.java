package InterviewQuestions.preperations.np_mid_term_1.log_processor;
...
class LogSystem<T extends ILog & Comparable<T>> {
    ...
    public LogSystem(ArrayList<T> elems) {
        this.logsList = elems;
    }
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = (logsList) -> logsList.stream()
                .filter(log -> log.getType().contains("INFO"))
                .collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<T> secondLogProcessor = (logsList) -> logsList.stream()
